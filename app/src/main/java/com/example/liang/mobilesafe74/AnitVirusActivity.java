...
import com.example.liang.mobilesafe74.engine.VirusDao;
...
public class AnitVirusActivity extends AppCompatActivity {
    ...
    private ArrayList<ScanInfo> virusScanInfoList;
    ...
    private void checkVirus() {
        new Thread(){
            ...
            public void run() {
                ...
                //2.获取所有应用程序签名文件  PackageManager.GET_SIGNATURES已安装应用的签名文件
                List<PackageInfo> packageInfosList = pm.getInstalledPackages(PackageManager.GET_SIGNATURES+PackageManager.GET_UNINSTALLED_PACKAGES);
                //创建记录病毒的集合
                virusScanInfoList = new ArrayList<ScanInfo>();
                //创建记录所有应用的集合
                List<ScanInfo> scanInfoList = new ArrayList<ScanInfo>();
                //设置进度条的最大值
                pb_bar.setMax(packageInfosList.size());
                //3.遍历应用的集合
                for (PackageInfo packageInfo:packageInfosList){
                    ...
                    //8.在子线程中发送消息，告知主线程更新UI（1.顶部扫描应用的名称；2.扫描过程中往线性布局中添加view）
